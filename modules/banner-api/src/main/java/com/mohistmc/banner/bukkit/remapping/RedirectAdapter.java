...
public class RedirectAdapter implements PluginTransformer {
    ...
    public static void scanMethod(byte[] bytes) {
        ...
        BannerMCStart.LOGGER.debug(MARKER, "Scanning {}", reader.getClassName());
                    ...
                    if (METHOD_MODIFY.containsKey(key) || METHOD_REDIRECT.containsKey(key)) {
                        try {
                            ...
                            if (target != null) {
                                ...
                                if (bridge != null) {
                                    BannerMCStart.LOGGER.debug(MARKER, "Creating bridge handler {}/{}{} to {}", node.name, method.name, method.desc, methodToString(target));

...
class LibraryLoader {
    ...
    public ClassLoader createLoader(@NotNull PluginDescriptionFile desc) {
        ...
        BannerMCStart.LOGGER.info(I18n.as("spigot.lib.loading"), desc.getName(), desc.getLibraries().size());
        ...
        BannerMCStart.LOGGER.info(I18n.as("spigot.lib.loading.extra"), desc.getName(), newDependencies.size() - desc.getLibraries().size());
        ...
        for (File file : libraries) {
            try {
                ...
                BannerMCStart.LOGGER.info(I18n.as("spigot.lib.loaded"), desc.getName(), file);
    ...
    public boolean has(Dependency dependency) {
        ...
        if (file.exists()) {
            if (!dependencyIgnoreVersion.contains(dependency.toIgnoreVersion())) {
                libraries.add(file);
                dependencyIgnoreVersion.add(dependency.toIgnoreVersion());
                BannerMCStart.LOGGER.info(I18n.as("spigot.lib.found"), dependency.name, file);
                return true;
            }
        }
        return false;

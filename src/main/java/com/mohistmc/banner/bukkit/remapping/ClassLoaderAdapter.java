...
public class ClassLoaderAdapter implements PluginTransformer {
    ...
    public void handleClass(ClassNode node, ClassLoaderRemapper remapper) {
            ...
            for (AbstractInsnNode insnNode : methodNode.instructions) {
                if (insnNode.getOpcode() == Opcodes.NEW) {
                    ...
                    if (replace != null) {
                        ...
                        BannerMCStart.LOGGER.debug(MARKER, "Found new {}/{} call in {} {}", typeInsnNode.desc, ((MethodInsnNode) next).name + ((MethodInsnNode) next).desc, node.name, methodNode.name + methodNode.desc);
        ...
        if (info == null) return;
        BannerMCStart.LOGGER.debug(MARKER, "Transforming classloader class {}", node.name);
        if (!info.remapping) {
            implementIntf(node);
        }
        for (MethodNode methodNode : node.methods) {
    ...
    private void implementIntf(ClassNode node) {
        BannerMCStart.LOGGER.debug(MARKER, "Implementing RemappingClassLoader for class {}", node.name);

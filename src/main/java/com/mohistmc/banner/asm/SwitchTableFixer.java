...
public class SwitchTableFixer implements Implementer, Function<byte[], byte[]> {
    ...
    private boolean inject1(ClassNode node, MethodNode method) {
        if (Modifier.isStatic(method.access) && (method.access & Opcodes.ACC_SYNTHETIC) != 0 && method.desc.equals("()[I")) {
            ...
            BannerMain.LOGGER.debug(MARKER, "Candidate switch enum method {} class {}", method.name + method.desc, node.name);
            ...
            if (fieldInsnNode != null && enumType != null) {
                BannerMain.LOGGER.debug(MARKER, "Find switch(enum {}) table method {} in class {}", enumType, method.name + method.desc, node.name);
                ...
                BannerMain.LOGGER.debug(MARKER, "Inject method in method {}:{}, switch table field is {}", node.name, method.name + method.desc, fieldInsnNode.name + fieldInsnNode.desc);
                return true;
            }
        }
        return false;
    ...
    public static int[] fillSwitchTable1(int[] arr, Class<? extends Enum<?>> cl) {
        BannerMain.LOGGER.debug(MARKER, "Filling switch table for {}", cl);
